...
public struct ReadResult: PostcardEncodable, Sendable {
...
public enum ItemType: PostcardEncodable, Sendable {
    case file
    case directory
    case symlink
...
public struct CreateResult: PostcardEncodable, Sendable {
...
public struct VfsResult: PostcardEncodable, Sendable {
    public var error: Int32
...
public struct WriteResult: PostcardEncodable, Sendable {
...
public struct ItemAttributes: PostcardEncodable, Sendable {
    public var item_id: UInt64
...
public struct GetAttributesResult: PostcardEncodable, Sendable {
...
public struct DirEntry: PostcardEncodable, Sendable {
...
public struct ReadDirResult: PostcardEncodable, Sendable {
...
public struct LookupResult: PostcardEncodable, Sendable {
    public var item_id: UInt64
    public var item_type: ItemType
    public var error: Int32
...
public struct SetAttributesParams: PostcardEncodable, Sendable {
    public var mode: UInt32?
    public var modified_time: UInt64?
...
public actor VfsClient {
    ...
    public func getAttributes(_ item_id: UInt64) async throws -> GetAttributesResult {
        ...
        NSLog("[VfsClient] getAttributes called for item_id=%llu", item_id)

        let response = try await client.call(methodId: 0x7DFD6287, requestPayload: encoder.bytes)

        // Log raw bytes immediately after receiving
        let hexDump = response.map { String(format: "%02x", $0) }.joined(separator: " ")
        NSLog("[VfsClient] getAttributes response (%d bytes): %@", response.count, hexDump)

        guard response.count >= 2 else {
            NSLog("[VfsClient] FATAL: Response too short")
            fatalError("Response too short (\(response.count) bytes): [\(hexDump)]")
        }

        var responseData = Data(response)
        let attrs_item_id = UInt64(try decodeVarint(from: &responseData))
        NSLog("[VfsClient] decoded attrs_item_id=%llu", attrs_item_id)

        let attrs_item_type_index = try decodeVarint(from: &responseData)
        NSLog("[VfsClient] decoded attrs_item_type_index=%llu", attrs_item_type_index)
        ...
        default:
            NSLog("[VfsClient] FATAL: Unknown ItemType=%llu, item_id=%llu, bytes=%@", attrs_item_type_index, attrs_item_id, hexDump)
            fatalError("ItemType=\(attrs_item_type_index), item_id=\(attrs_item_id), response=[\(hexDump)]")
        }
